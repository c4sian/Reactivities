...
export default function ProfilePhotos() {
    ...
    if (!photos) return <Typography>No photos were found for this user.</Typography>
    ...
    return (
        <Box>
            ...
            {editMode ? (
                ...
                <>
                    {photos.length === 0 ? (
                        <Typography>You haven't added any photos yet.</Typography>
                    ) : (
                                ...
                                <ImageListItem key={item.id}>
                                    <img
                                        srcSet={`${item.url.replace(
                                            '/upload/',
                                            '/upload/w_500,h_500,c_fill,f_auto,dpr_2,g_face/'
                                        )}`}
                                        src={`${item.url.replace(
                                            '/upload/',
                                            '/upload/w_500,h_500,c_fill,f_auto,g_face/'
                                        )}`}
